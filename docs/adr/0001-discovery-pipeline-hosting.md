...
**Benchmark artifact summary.** The benchmark was a one-off Python evaluation used only to choose the initial hosted embedding provider. It compared HF `sentence-transformers/all-MiniLM-L6-v2`, Voyage `voyage-3-lite`, and OpenAI `text-embedding-3-small` over a 48-album hand-curated corpus and 14 representative natural-language queries. The report was read manually rather than scored automatically because the question was qualitative retrieval fit, not benchmark leaderboard performance. The strongest queries showed all three providers could retrieve obvious reference matches, while the weakest queries exposed corpus-thinness and prompt-shape issues more than provider quality. Voyage still won the v1 decision on operational grounds: free-tier headroom, no cold-load behavior, and no account-balance gate. The runnable benchmark scripts and local result artifacts were removed after the production Go clients landed to avoid confusion with runtime `DISCOVERY_*` environment variables.
...
1. [x] **Embedding provider benchmark.** *Resolved 2026-05-08:* Voyage `voyage-3-lite` selected. All three providers tied on quality across the curated corpus and queries; operational properties decided it. The one-off runnable benchmark artifacts were removed after the production Go clients landed to avoid confusion with runtime `DISCOVERY_*` environment variables. See Decision section for full rationale and methodology caveats.
...
9. [x] **Update `BACKLOG.md`.** *Resolved 2026-05-08:* In-progress entry added pointing to this ADR and the implementation plan, with a note that "Related Artists" and "themed browsing" can ride on the same embedding table.
