...
- **2026-01-11:** Implemented SSR proxy architecture for production deployment: Added `http-proxy-middleware` to Angular SSR Express server to proxy all `/api/*` requests to Go backend (configurable via `API_BASE_URL` environment variable). Updated all frontend services (`SearchService`, `ArtistService`, `AlbumService`) to use relative `/api/*` paths instead of hardcoded `localhost:8080` URLs. This eliminates CORS issues, enables single-origin deployments, and supports environment-agnostic configuration. Angular SSR now properly trusts reverse proxies (Render/Fly/Vercel) via `trust proxy` setting. Proxy automatically strips `/api` prefix when forwarding to backend. Local development defaults to `http://localhost:8080`; production sets `API_BASE_URL` to internal Go service URL.
